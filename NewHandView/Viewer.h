...
struct VisData {
	...
	void SS_set(vector<PointType> &disVertices, vector<PatchType> &disPatches) {
		for (int i = 0; i < disPatches.size(); i++) {
			indices[3 * i] = disPatches[i].v_idx(0);
			indices[3 * i + 1] = disPatches[i].v_idx(1);
			indices[3 * i + 2] = disPatches[i].v_idx(2);
		}
		for (int i = 0; i < disVertices.size(); i++) {
			this->vertices[3 * i] = float(disVertices[i].position.x);
			this->vertices[3 * i + 1] = float(disVertices[i].position.y);
			this->vertices[3 * i + 2] = float(disVertices[i].position.z);

			this->colors[3 * i] = disVertices[i].color(0);
			this->colors[3 * i + 1] = disVertices[i].color(1);
			this->colors[3 * i + 2] = disVertices[i].color(2);
		}
	}
